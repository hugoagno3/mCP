...
##################
  average_matrices <- function(mat1, mat2) {
    if (!all(dim(mat1) == dim(mat2))) {
      stop("Matrices must have the same dimensions.")
    }
    # Calculate element-wise means
    averaged_matrix <- (mat1 + mat2) / 2
    result_matrix <- matrix(averaged_matrix, ncol = ncol(mat1),
                            dimnames = list(rownames(mat1), colnames(mat1)))
    return(result_matrix)
  }
  result_matrix <- average_matrices(res_matrix, res_matrix_test)
  binary_result <- ifelse(result_matrix == 2, 1, 0)
