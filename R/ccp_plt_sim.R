                                                              ...
                                                              list(x = 13, label = "720 KDa"))) 
{
  ...
  for (i in seq_along(complex_list)) {
    ...
    if (sum(dim(corMat)) == 0 | nrow(data) < N_fractions | 
        all(data$Intensity[which(!is.na(data$Intensity))] == 
            0)) {
      next
    }
    pectores <- corMat
    pectores <- gdata::unmatrix(pectores)
    pere <- as.data.frame(pectores[pectores > ee[i] & pectores < 
                                     1])
    colnames(pere)[1] <- "Cor"
    ...
    tri[is.na(tri)] <- 0
    if (any(tri > ee[i])) {
      vere <- data.frame(Hits = length(which(tri > ee[i])))
      c_counter <- c_counter + 1
      if (relative) {
        data <- data %>% group_by(protein_id) %>% mutate(Intensity = Intensity/max(Intensity, 
                                                                                   na.rm = TRUE))
      }
      p <- ggplot2::ggplot(data, ggplot2::aes(x = SEC_FR, 
                                              y = Intensity, ggplot2::ggtitle(complex_name), 
                                              col = prot_name)) + ggplot2::geom_line() + ggplot2::scale_x_continuous(name = "Fractions", 
      ...
      plots_list <- c(plots_list, list(c(list(p), list(vere), 
                                         list(perek_2))))
  ...
  if (tolower(format) == "pdf" & heat_map) {
    pdf(file = paste0("heatmap_", output_name, ".", format), 
        width = 8, height = 6)
    ...
    for (i in seq_along(complex_list)) {
      ...
      if (any(tri > ee[i])) {
        ...
        if (heatmap_seaborn) {
          ...
          if (sum(apply(complex_list[[i]][["CorMat_rrr"]][, 
                                                          -1], 1, duplicated)) == 0) {
            d <- corrr::network_plot(complex_list[[i]][["CorMat_rrr"]], 
                                     min_cor = 0.3)
          }
          else {
            colname <- colnames(complex_list[[i]][["CorMat_rrr"]])[1]
            d <- as.data.frame(complex_list[[i]][["CorMat_rrr"]]) %>% 
              ggplot(aes(y = {
                {
                  colname
                }
              })) + geom_bar() + ggtitle("Error on the matrix, please try manually")
          }
        }
        cp_names <- c(cp_names, as.character(data$complex_name[1]))
        if (tolower(format) == "pdf" & heat_map) {
        ...
        if (heatmap_seaborn) {
          plots_list_heatmaps <- c(plots_list_heatmaps, 
                                   list(c(list(d))))
        }
      }
    }
